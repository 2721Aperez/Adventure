...
int main() {
    ...
    while (!exitGame) {
        ...
        while (startGame) {
            ...
            std::cout << "You come to the end of the hallway, would you like to turn (l) left or (r) right? " << std::endl;
            ...
            while (!bossEntered) { //check if player is dead in loop as well
                switch (generatedRoom) {
                    ...
                    case 'h': //hallway
                         ...
                         */
                        std::cout << "A random enemy minion appears as you traverse the halls of ZugZwang!" << std::endl;
                        std::cout << "You must defeat the minion to proceed." << std::endl;
                        if (generateMinionEncounter(*adventurer)) {
                            //play defeats minion
            ...
            if (potion == 'A' || potion == 'a') {
                ...
                for (int i = 0; i < Adventurer.backpack.potion.size(); i++) {
                    if (Adventurer.backpack.potion[i]->potionType == "attack") {
                        potion_index = i;
                        potionFound = true;
                        std::cout << "potion at: " << potion_index << " " << Adventurer.backpack.potion[i]->potionType << std::endl;
                    }
                }
                if (potionFound) {
...
void generateLoot(Player &Adventurer) {
    ...
    switch (randItem) {
        case 0: //Divine Axe
            ...
            std::cin >> take_choice;
            if (take_choice == 'Y' || take_choice == 'y') {
...
bool generateBossEncounter(Player &Adventurer) {
    ...
    int potion_damage;
    char potion_choice;
    bool currentBattle = true;
    bool hasTransformed = false;
    EnemyFactory enemyType;
    Enemy *boss = enemyType.make_chooser("boss");
    std::cout << "Welcome to the deepest chamber within ZugZwang" << std::endl;
    std::cout << "Not many have made it this far, but no one has yet defeated me!" << std::endl;
    while (currentBattle) {
        if (Adventurer.backpack.potion.empty()) {
            //no potions to use
            //std::cout << "You have no potions for this battle." << std::endl;
            potion_damage = 0;
        }
        else {
            ...
            if (potion_choice == 'A' || potion_choice == 'a') {
                //check for attack potion
                bool potionFound = false;
                int potion_index;
                for (int i = 0; i < Adventurer.backpack.potion.size(); i++) {
            ...
            } else if (potion_choice == 'H' || potion_choice == 'h') {
                bool potionFound = false;
                int potion_index;
                for (int i = 0; i < Adventurer.backpack.potion.size(); i++) {
                    if (Adventurer.backpack.potion[i]->potionType == "health") {
        ...
        std::cout << "Boss HP: " << boss->getHP() << std::endl;
        std::cout << "Adventurer HP: " << Adventurer.getHP() << std::endl;
